        ...
        <Pressable style={[styles.sheet, { backgroundColor: palette.background }]} onPress={() => undefined}>
          ...
          <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.body}>
            ...
            <View style={styles.quickSection}>
              <Text style={[styles.compactLabel, { color: palette.muted }]}>{t("優先度", "Priority")}</Text>
              ...
              <Text style={[styles.compactLabel, { color: palette.muted }]}>{t("期限", "Due date")}</Text>
              <TouchableOpacity accessibilityRole="button" onPress={() => setDatePickerOpen(true)} style={[styles.dateButton, { backgroundColor: palette.surface, borderColor: palette.border }]}><MaterialIcons name="calendar-today" size={17} color={palette.primary} /><Text style={[styles.dateText, { color: palette.text }, !dueDate && { color: palette.muted }]} numberOfLines={1}>{dueDate ? formatJapaneseDate(dueDate, language) : t("必要な場合だけ選択", "Choose only if needed")}</Text><MaterialIcons name="chevron-right" size={19} color={palette.muted} /></TouchableOpacity>
            </View>

            <TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: memoOpen }} onPress={() => setMemoOpen((value) => !value)} style={[styles.disclosureRow, { backgroundColor: palette.elevated }]}>
              <MaterialIcons name={memoOpen ? "edit-note" : "note-add"} size={19} color={palette.primary} />
              <View style={styles.disclosureCopy}>
                <Text style={[styles.disclosureTitle, { color: palette.text }]}>{memo ? t("メモ", "Note") : t("メモを追加", "Add a note")}</Text>
                {memo && !memoOpen ? <Text style={[styles.disclosurePreview, { color: palette.muted }]} numberOfLines={1}>{memo}</Text> : null}
              </View>
            ...
            <TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: subtasksOpen }} onPress={() => setSubtasksOpen((value) => !value)} style={[styles.disclosureRow, { backgroundColor: palette.elevated }]}>
              ...
              <View style={styles.disclosureCopy}>
                <Text style={[styles.disclosureTitle, { color: palette.text }]}>{subtasks.length ? t(`サブタスク ${completedSubtasks}/${subtasks.length}`, `Subtasks ${completedSubtasks}/${subtasks.length}`) : t("サブタスクを追加", "Add subtasks")}</Text>
                {!subtasks.length ? <Text style={[styles.disclosurePreview, { color: palette.muted }]}>{t("大きな作業を小さく分ける", "Break the task into smaller steps")}</Text> : null}
              </View>
...
const styles = StyleSheet.create({
  ...
  smallOption: { flex: 1, minHeight: 40, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5, borderRadius: 11, borderWidth: 1, paddingHorizontal: 3 },
  priorityDot: { width: 7, height: 7, borderRadius: 4 },
  smallOptionText: { fontSize: 11, fontWeight: "800" },
  dateButton: { minHeight: 43, flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 12, borderWidth: 1, paddingHorizontal: 12, marginBottom: 8 },
  dateText: { flex: 1, fontSize: 13, fontWeight: "700" },
  disclosureRow: { minHeight: 45, flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 12, paddingHorizontal: 12, marginBottom: 7 },
  disclosureCopy: { flex: 1, minWidth: 0 },
  disclosureTitle: { fontSize: 13, lineHeight: 18, fontWeight: "800" },
  disclosurePreview: { fontSize: 11, lineHeight: 15, marginTop: 1 },
  memoInput: { minHeight: 72, borderRadius: 12, borderWidth: 1, fontSize: 13, lineHeight: 18, paddingHorizontal: 12, paddingTop: 10, marginBottom: 7 },
  ...
  subtaskInput: { flex: 1, minHeight: 41, borderRadius: 11, borderWidth: 1, fontSize: 13, paddingHorizontal: 11 },
  subtaskAddButton: { width: 41, height: 41, borderRadius: 11, alignItems: "center", justifyContent: "center" },
  requiredLabel: { marginTop: 2 },
  requiredOption: { minHeight: 55, flexDirection: "row", alignItems: "center", gap: 9, borderRadius: 13, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 6, marginBottom: 9 },
