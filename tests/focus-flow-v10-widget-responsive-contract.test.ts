...
describe("Widgetのサイズ・連続リスト・必須表示契約", () => {
  it("リサイズCallbackとAndroid 12 responsive mapでsmall・medium・largeを1/2/3行へ切り替える", () => { const provider = source("plugins", "native", "android", "kotlin", "FocusFlowWidgetProvider.kt"); ["onAppWidgetOptionsChanged", "rememberWidgetBucket", "createWidgetViews", "WidgetBucket(1", "WidgetBucket(2", "WidgetBucket(3", "WidgetBucket(5", "manager.updateAppWidget(id, createWidgetViews"].forEach((text) => expect(provider).toContain(text)); expect(provider).not.toContain("responsiveWidgetViews"); expect(provider).not.toContain("RemoteViews(mappings)"); });
