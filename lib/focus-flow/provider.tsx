...
export type TodoInput = { title: string; priority: Priority; dueDate?: string; isRequired: boolean; requiredWindowMode?: RequiredWindowMode; requiredScheduleIds?: string[] };
...
function normalizedTodo(todo: Todo, legacyIds: Set<string>, availableScheduleIds?: Set<string>): Todo {
  ...
  return { ...todo, isRequired, ...normalizedRequiredWindow(isRequired, todo.requiredWindowMode, todo.requiredScheduleIds, availableScheduleIds), progressUnit: "check", targetValue: 1, progressValue: todo.completed ? 1 : 0, repeatRule: "none", subtasks: [], timerStartedAt: undefined, earlyCompletionAt: undefined };
}
...
export function FocusFlowProvider({ children }: { children: ReactNode }) {
  ...
  const addTodo = useCallback((input: TodoInput): MutationResult => {
    ...
    const todo: Todo = { id: createId("todo"), title, priority: input.priority, dueDate: input.dueDate, isRequired: input.isRequired, ...normalizedRequiredWindow(input.isRequired, input.requiredWindowMode, input.requiredScheduleIds, new Set(data.gateConfig.schedules.map((schedule) => schedule.id))), completed: false, progressUnit: "check", targetValue: 1, progressValue: 0, repeatRule: "none", subtasks: [], createdAt: new Date().toISOString() };
    commit((current) => ({ ...current, todos: [todo, ...current.todos] })); return { ok: true };
  }, [commit, data.todos, isPlus]);

  const updateTodo = useCallback((id: string, input: TodoInput): MutationResult => { const title = input.title.trim(); if (!title) return { ok: false }; commit((current) => ({ ...current, todos: current.todos.map((todo) => todo.id !== id ? todo : { ...todo, title, priority: input.priority, dueDate: input.dueDate, isRequired: input.isRequired, ...normalizedRequiredWindow(input.isRequired, input.requiredWindowMode, input.requiredScheduleIds, new Set(current.gateConfig.schedules.map((schedule) => schedule.id))), progressUnit: "check", targetValue: 1, progressValue: todo.completed ? 1 : 0, repeatRule: "none", subtasks: [], timerStartedAt: undefined, earlyCompletionAt: undefined }) })); return { ok: true }; }, [commit]);
  ...
  const toggleSubtask = useCallback((_todoId: string, _subtaskId: string): MutationResult => ({ ok: false }), []);
