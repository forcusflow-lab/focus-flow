...
export function FocusFlowProvider({ children }: { children: ReactNode }) {
  ...
  const isPlus = Boolean(data.displaySettings.plusEntitlement && plusStatus.active);

  const addTodo = useCallback((input: TodoInput): MutationResult => {
    const title = input.title.trim(); if (!title) return { ok: false };
    if (isFreeItemLimitReached(data.todos.length, isPlus)) return { ok: false, reason: "FREE_LIMIT_REACHED" };
  ...
  const addHabit = useCallback((input: HabitInput): MutationResult => { const title = input.title.trim(); if (!title) return { ok: false }; if (isFreeItemLimitReached(data.habits.length, isPlus)) return { ok: false, reason: "FREE_LIMIT_REACHED" }; const habit: Habit = { id: createId("habit"), title, color: input.color, goalPerWeek: Math.min(Math.max(input.goalPerWeek, 1), 7), isRequired: input.isRequired, completedDates: [], progressUnit: input.progressUnit ?? "check", targetValue: Math.max(input.targetValue ?? 1, 1), dailyProgress: {}, timerStartedAtByDate: {}, earlyCompletionDates: [], createdAt: new Date().toISOString() }; commit((current) => ({ ...current, habits: [habit, ...current.habits] })); return { ok: true }; }, [commit, data.habits.length, isPlus]);
  const updateHabit = useCallback((id: string, input: HabitInput): MutationResult => { const title = input.title.trim(); if (!title) return { ok: false }; commit((current) => ({ ...current, habits: current.habits.map((habit) => habit.id === id ? { ...habit, title, color: input.color, goalPerWeek: Math.min(Math.max(input.goalPerWeek, 1), 7), isRequired: input.isRequired, progressUnit: input.progressUnit ?? "check", targetValue: Math.max(input.targetValue ?? 1, 1), timerStartedAtByDate: input.progressUnit === "minutes" ? habit.timerStartedAtByDate : {}, earlyCompletionDates: input.progressUnit === "minutes" ? habit.earlyCompletionDates : [] } : habit) })); return { ok: true }; }, [commit]);
  ...
  useEffect(() => { if (Platform.OS === "web" || !connected) return; void fetchProducts({ skus: [PLUS_PRODUCT_ID], type: "subs" }); void fetchProducts({ skus: [EARLY_COMPLETION_PRODUCT_ID], type: "in-app" }); void getActiveSubscriptions([PLUS_PRODUCT_ID]); }, [connected, fetchProducts, getActiveSubscriptions]);
  useEffect(() => { if (Platform.OS === "web" || !connected) return; const active = activeSubscriptions.some((subscription) => subscription.productId === PLUS_PRODUCT_ID && subscription.isActive); applyPlusStatus({ status: active ? "active" : "eligible", active, productId: PLUS_PRODUCT_ID, price: plusProduct?.displayPrice }); setEarlyCompletionStatus({ status: earlyCompletionProduct ? "inactive" : "unavailable", productId: EARLY_COMPLETION_PRODUCT_ID, price: earlyCompletionProduct?.displayPrice, reason: earlyCompletionProduct ? undefined : "STORE_PRODUCT_UNAVAILABLE" }); }, [activeSubscriptions, applyPlusStatus, connected, earlyCompletionProduct, plusProduct?.displayPrice]);
  ...
  const applyWidgetCompletions = useCallback(async () => { const actions = await consumeWidgetCompletions(); if (!actions.length) return; commit((current) => { const today = dayKey(); const todoIds = new Set(actions.filter((action) => action.kind === "todo").map((action) => action.id)); const habitIds = new Set(actions.filter((action) => action.kind === "habit").map((action) => action.id)); const todos = current.todos.map((todo) => { if (!todoIds.has(todo.id) || isTodoAchieved(todo) || !isTodoRequiredForGate(todo, current.gateConfig.autoRequireDueToday) || (isTimedTodo(todo) && !isTodoTimeReady(todo))) return todo; const completed: Todo = { ...todo, completed: true, completedAt: new Date().toISOString(), progressValue: todo.progressUnit === "check" ? todo.progressValue : todo.targetValue, subtasks: getTodoSubtasks(todo).map((subtask) => ({ ...subtask, completed: true })) }; return maybeAdvanceRecurring(completed); }); const habits = current.habits.map((habit) => { if (!habitIds.has(habit.id) || !habit.isRequired || ((habit.progressUnit ?? "check") === "minutes" && !isHabitTimeReady(habit, today))) return habit; const completedDates = Array.from(new Set([...habit.completedDates, today])).sort(); return { ...habit, completedDates, dailyProgress: { ...(habit.dailyProgress ?? {}), [today]: habit.targetValue ?? 1 } }; }); return { ...current, todos, habits }; }); }, [commit]);
