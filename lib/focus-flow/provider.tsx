...
import { markReviewPromptShown, recordMilestoneAndCheckPrompt } from "./review-prompt";
...
export type TodoInput = { title: string; memo?: string; color?: string; priority: Priority; dueDate?: string; isRequired: boolean; requiredWindowMode?: RequiredWindowMode; requiredScheduleIds?: string[]; subtasks?: TodoSubtask[] };
export type HabitInput = { title: string; color: string; goalPerWeek: number; targetDays?: number[]; isRequired: boolean; requiredWindowMode?: RequiredWindowMode; requiredScheduleIds?: string[]; progressUnit?: ProgressUnit; targetValue?: number };
...
type FocusFlowContextValue = FocusFlowData & {
  ...
  triggerMilestoneReview: () => Promise<boolean>;
};
...
export function FocusFlowProvider({ children }: { children: ReactNode }) {
  ...
  const addTodo = useCallback((input: TodoInput): MutationResult => {
    ...
    const todo: Todo = { id: createId("todo"), title, memo: input.memo?.trim() || undefined, color: input.color, priority: input.priority, dueDate: input.dueDate, isRequired: input.isRequired, ...normalizedRequiredWindow(input.isRequired, input.requiredWindowMode, input.requiredScheduleIds, new Set(data.gateConfig.schedules.map((schedule) => schedule.id))), completed: false, progressUnit: "check", targetValue: 1, progressValue: 0, repeatRule: "none", subtasks: input.subtasks ?? [], createdAt: new Date().toISOString() };
    commit((current) => ({ ...current, todos: [todo, ...current.todos] })); return { ok: true };
  }, [commit, data.todos, isPlus]);

  const updateTodo = useCallback((id: string, input: TodoInput): MutationResult => { const title = input.title.trim(); if (!title) return { ok: false }; commit((current) => ({ ...current, todos: current.todos.map((todo) => { if (todo.id !== id) return todo; const nextSubtasks = input.subtasks ?? getTodoSubtasks(todo); const hasSubtasks = nextSubtasks.length > 0; const allCompleted = hasSubtasks && nextSubtasks.every((s) => s.completed); let completed = todo.completed; let completedAt = todo.completedAt; if (hasSubtasks) { if (allCompleted && !todo.completed) { completed = true; completedAt = new Date().toISOString(); } else if (!allCompleted && todo.completed) { completed = false; completedAt = undefined; } } return { ...todo, title, memo: input.memo?.trim() || undefined, color: input.color !== undefined ? input.color : todo.color, priority: input.priority, dueDate: input.dueDate, isRequired: input.isRequired, ...normalizedRequiredWindow(input.isRequired, input.requiredWindowMode, input.requiredScheduleIds, new Set(current.gateConfig.schedules.map((schedule) => schedule.id))), completed, completedAt, progressUnit: "check", targetValue: 1, progressValue: completed ? 1 : 0, repeatRule: "none", subtasks: input.subtasks ?? getTodoSubtasks(todo), timerStartedAt: undefined, earlyCompletionAt: undefined }; }) })); return { ok: true }; }, [commit]);
  ...
  const toggleSubtask = useCallback((todoId: string, subtaskId: string): MutationResult => { let result: MutationResult = { ok: false }; commit((current) => ({ ...current, todos: current.todos.map((todo) => { if (todo.id !== todoId) return todo; const subtasks = getTodoSubtasks(todo).map((subtask) => subtask.id === subtaskId ? { ...subtask, completed: !subtask.completed } : subtask); result = { ok: true }; const hasSubtasks = subtasks.length > 0; const allCompleted = hasSubtasks && subtasks.every((s) => s.completed); let completed = todo.completed; let completedAt = todo.completedAt; if (hasSubtasks) { if (allCompleted && !todo.completed) { completed = true; completedAt = new Date().toISOString(); } else if (!allCompleted && todo.completed) { completed = false; completedAt = undefined; } } return { ...todo, completed, completedAt, progressValue: completed ? 1 : 0, subtasks }; }) })); return result; }, [commit]);
  const deleteTodo = useCallback((id: string) => commit((current) => ({ ...current, todos: current.todos.filter((todo) => todo.id !== id) })), [commit]);

  const addHabit = useCallback((input: HabitInput): MutationResult => { const title = input.title.trim(); if (!title) return { ok: false }; if (isFreeItemLimitReached(data.habits.length, isPlus)) return { ok: false, reason: "FREE_LIMIT_REACHED" }; const targetDays = input.targetDays && input.targetDays.length > 0 ? input.targetDays : [1, 2, 3, 4, 5, 6, 0]; const goalPerWeek = input.targetDays ? input.targetDays.length : Math.min(Math.max(input.goalPerWeek, 1), 7); const habit: Habit = { id: createId("habit"), title, color: input.color, goalPerWeek, targetDays, isRequired: input.isRequired, ...normalizedRequiredWindow(input.isRequired, input.requiredWindowMode, input.requiredScheduleIds, new Set(data.gateConfig.schedules.map((schedule) => schedule.id))), completedDates: [], progressUnit: input.progressUnit ?? "check", targetValue: Math.max(input.targetValue ?? 1, 1), dailyProgress: {}, timerStartedAtByDate: {}, timerElapsedSecondsByDate: {}, earlyCompletionDates: [], createdAt: new Date().toISOString() }; commit((current) => ({ ...current, habits: [habit, ...current.habits] })); return { ok: true }; }, [commit, data.gateConfig.schedules, data.habits.length, isPlus]);
  const updateHabit = useCallback((id: string, input: HabitInput): MutationResult => { const title = input.title.trim(); if (!title) return { ok: false }; commit((current) => ({ ...current, habits: current.habits.map((habit) => { if (habit.id !== id) return habit; const targetDays = input.targetDays !== undefined ? input.targetDays : habit.targetDays; const goalPerWeek = input.targetDays ? input.targetDays.length : Math.min(Math.max(input.goalPerWeek, 1), 7); return { ...habit, title, color: input.color, goalPerWeek, targetDays, isRequired: input.isRequired, ...normalizedRequiredWindow(input.isRequired, input.requiredWindowMode, input.requiredScheduleIds, new Set(current.gateConfig.schedules.map((schedule) => schedule.id))), progressUnit: input.progressUnit ?? "check", targetValue: Math.max(input.targetValue ?? 1, 1), timerStartedAtByDate: input.progressUnit === "minutes" ? habit.timerStartedAtByDate : {}, timerElapsedSecondsByDate: input.progressUnit === "minutes" ? habit.timerElapsedSecondsByDate : {}, earlyCompletionDates: input.progressUnit === "minutes" ? habit.earlyCompletionDates : [] }; }) })); return { ok: true }; }, [commit]);
  ...
  const closeReviewPrompt = useCallback(() => setReviewPromptVisible(false), []);
  const triggerMilestoneReview = useCallback(async (): Promise<boolean> => {
    try {
      const shouldPrompt = await recordMilestoneAndCheckPrompt();
      if (shouldPrompt) {
        openReviewPrompt();
        return true;
      }
  ...
  const value = useMemo(() => ({ ...data, isReady, isPlus, paywallVisible, openPaywall, closePaywall, reviewPromptVisible, openReviewPrompt, closeReviewPrompt, triggerMilestoneReview, addTodo, updateTodo, toggleTodo, adjustTodoProgress, toggleSubtask, deleteTodo, addHabit, updateHabit, toggleHabit, startHabitTimer, pauseHabitTimer, adjustHabitProgress, deleteHabit, addMemo, updateMemo, deleteMemo, addFocusSession, setGateConfig, canSelectBlockedApp, setDisplaySettings, clearAllData, plusStatus, earlyCompletionStatus, earlyCompletionPrice: earlyCompletionProduct?.displayPrice, refreshPlusStatus, purchasePlus, restorePlus, managePlus, purchaseEarlyCompletion }), [data, isReady, isPlus, paywallVisible, openPaywall, closePaywall, reviewPromptVisible, openReviewPrompt, closeReviewPrompt, triggerMilestoneReview, addTodo, updateTodo, toggleTodo, adjustTodoProgress, toggleSubtask, deleteTodo, addHabit, updateHabit, toggleHabit, startHabitTimer, pauseHabitTimer, adjustHabitProgress, deleteHabit, addMemo, updateMemo, deleteMemo, addFocusSession, setGateConfig, canSelectBlockedApp, setDisplaySettings, clearAllData, plusStatus, earlyCompletionStatus, earlyCompletionProduct?.displayPrice, refreshPlusStatus, purchasePlus, restorePlus, managePlus, purchaseEarlyCompletion]);
