...
export function FocusFlowProvider({ children }: { children: ReactNode }) {
  ...
  const applyWidgetActions = useCallback(async () => {
    ...
    commit((current) => {
      ...
      const todos = current.todos.map((todo) => {
        ...
        const operation = operations?.at(-1)?.operation;
  ...
  const value = useMemo(() => ({ ...data, isReady, addTodo, updateTodo, toggleTodo, adjustTodoProgress, toggleSubtask, deleteTodo, addHabit, updateHabit, toggleHabit, startHabitTimer, adjustHabitProgress, deleteHabit, addMemo, updateMemo, deleteMemo, addFocusSession, setGateConfig, canSelectBlockedApp, setDisplaySettings, clearAllData, plusStatus, earlyCompletionStatus, earlyCompletionPrice: earlyCompletionProduct?.displayPrice, refreshPlusStatus, purchasePlus, restorePlus, managePlus, purchaseEarlyCompletion }), [data, isReady, addTodo, updateTodo, toggleTodo, adjustTodoProgress, toggleSubtask, deleteTodo, addHabit, updateHabit, toggleHabit, startHabitTimer, adjustHabitProgress, deleteHabit, addMemo, updateMemo, deleteMemo, addFocusSession, setGateConfig, canSelectBlockedApp, setDisplaySettings, clearAllData, plusStatus, earlyCompletionStatus, earlyCompletionProduct?.displayPrice, refreshPlusStatus, purchasePlus, restorePlus, managePlus, purchaseEarlyCompletion]);
