...
export async function syncAndroidGate(data: FocusFlowData) {
  ...
  const widgetItems = uniqueWidgetItems([
    ...orderedPendingTodos.map((todo) => ({ id: todo.id, title: todo.title, kind: "todo", required: true, gateRequired: requiredTodoIds.has(todo.id), stableOrder: data.todos.indexOf(todo) * 2, accentColor: todo.priority === "high" ? "#C24756" : todo.priority === "medium" ? "#B96B13" : "#3566B7", windowLabel: windowLabelFor(todo), subtaskCount: getTodoSubtasks(todo).length, timedLocked: timedLockedTodoIds.includes(todo.id), completed: false, canToggle: !timedLockedTodoIds.includes(todo.id) && todo.repeatRule === "none", progressUnit: todo.progressUnit ?? "check", progressValue: todo.progressValue ?? 0, targetValue: todo.targetValue ?? 1 })),
    ...
    ...completedTodoItems.map((todo) => ({ id: todo.id, title: todo.title, kind: "todo", required: true, gateRequired: requiredTodoIds.has(todo.id), stableOrder: data.todos.indexOf(todo) * 2, accentColor: todo.priority === "high" ? "#C24756" : todo.priority === "medium" ? "#B96B13" : "#3566B7", windowLabel: windowLabelFor(todo), subtaskCount: getTodoSubtasks(todo).length, timedLocked: false, completed: true, canToggle: todo.repeatRule === "none", progressUnit: "check", progressValue: 1, targetValue: 1 })),
