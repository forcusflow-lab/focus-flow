...
export async function syncAndroidGate(data: FocusFlowData) {
  ...
  const widgetItems = uniqueWidgetItems([
    ...orderedPendingTodos.map((todo) => ({ id: todo.id, title: todo.title, kind: "todo", required: true, gateRequired: requiredTodoIds.has(todo.id), stableOrder: data.todos.indexOf(todo) * 2, accentColor: todo.priority === "high" ? "#C24756" : todo.priority === "medium" ? "#B96B13" : "#3566B7", windowLabel: windowLabelFor(todo), timedLocked: timedLockedTodoIds.includes(todo.id), completed: false, canToggle: !timedLockedTodoIds.includes(todo.id) && todo.repeatRule === "none", progressUnit: todo.progressUnit ?? "check", progressValue: todo.progressValue ?? 0, targetValue: todo.targetValue ?? 1 })),
    ...
    ...(widgetCompletedDisplay === "dim" ? [
      ...completedTodoItems.map((todo) => ({ id: todo.id, title: todo.title, kind: "todo", required: true, gateRequired: requiredTodoIds.has(todo.id), stableOrder: data.todos.indexOf(todo) * 2, accentColor: todo.priority === "high" ? "#C24756" : todo.priority === "medium" ? "#B96B13" : "#3566B7", windowLabel: windowLabelFor(todo), timedLocked: false, completed: true, canToggle: todo.repeatRule === "none", progressUnit: todo.progressUnit ?? "check", progressValue: todo.targetValue ?? 1, targetValue: todo.targetValue ?? 1 })),
      ...completedHabitItems.map((habit) => ({ id: habit.id, title: habit.title, kind: "habit", required: true, gateRequired: requiredHabitIds.has(habit.id), stableOrder: data.habits.indexOf(habit) * 2 + 1, accentColor: habit.color, windowLabel: windowLabelFor(habit), timedLocked: false, completed: true, canToggle: true, progressUnit: habit.progressUnit ?? "check", progressValue: habit.dailyProgress?.[today] ?? habit.targetValue ?? 1, targetValue: habit.targetValue ?? 1, timerRunning: false, timerPaused: false })),
    ] : []),
