...
export async function syncAndroidGate(data: FocusFlowData) {
  ...
  const widgetItems = uniqueWidgetItems([
    ...
    ...completedTodoItems.map((todo) => ({ id: todo.id, title: todo.title, kind: "todo", required: true, gateRequired: requiredTodoIds.has(todo.id), stableOrder: data.todos.indexOf(todo) * 2, accentColor: todo.priority === "high" ? "#C24756" : todo.priority === "medium" ? "#B96B13" : "#3566B7", windowLabel: windowLabelFor(todo), timedLocked: false, completed: true, canToggle: todo.repeatRule === "none", progressUnit: "check", progressValue: 1, targetValue: 1 })),
