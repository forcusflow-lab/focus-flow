...
import { dayKey, formatJapaneseDate, getGateRuleSummaries, getGateSummary, getTodoAccentColor, weeklyHabitProgress, getHabitTimerProgress, getTodoDueStatus, getTodoSubtasks, habitStreak, habitTimerEndsAt, isHabitCompleteOn, isHabitScheduledOn, isHabitTimeReady, isTimedTodo, isTodoAchieved, isTodoEffectiveRequired, isTodoTimeReady, todoTimerEndsAt } from "./utils";
...
export async function syncAndroidGate(data: FocusFlowData) {
  ...
  const widgetItems = uniqueWidgetItems([
    ...
    ...completedTodoItems.map((todo) => ({ id: todo.id, title: todo.title, kind: "todo", required: true, gateRequired: requiredTodoIds.has(todo.id), stableOrder: data.todos.indexOf(todo) * 2, accentColor: getTodoAccentColor(todo), windowLabel: windowLabelFor(todo), ...todoWidgetMeta(todo), timedLocked: false, completed: true, canToggle: todo.repeatRule === "none", progressUnit: "check", progressValue: 1, targetValue: 1 })),
