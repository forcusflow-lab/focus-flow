...
| ID | 操作 | 合格条件 |
|---|---|---|
| A-01 | Todoを1件だけ作成してWidgetを更新する | 合格。表示は1行だけで、空の2行目・同じTodoの重複表示はない |
| A-02 | Todoを2件作成する | 合格。異なるTodoだけが上段・下段に1件ずつ表示される |
| A-03 | 上段、下段を個別にチェック・復元する | 合格。押した行だけが完了／復元され、もう一方へチェックが移らない |
| A-04 | 行本文、チェック、ヘッダー／空状態を押す | 合格。詳細、該当項目の状態変更、Todayが混線しない |
| A-05 | ライト／ダーク、文字サイズ3段階、サイズ変更を試す | 合格。20dpチェックが中央にあり、タイトル・補足・必須・時間帯を判読でき、文字切れ・重なりがない |

**実機受入結果:** 2026-08-26に利用者から「動作は完璧」との合格報告を受領した。上記A-01〜A-05を合格として記録する。v7は、Widget追加、重複表示、行状態混線、20dpチェック、詳細／Today導線、テーマ、文字サイズ、リサイズの実機品質ゲートを通過した。

この合格は本人用v7 APKに対する結果である。通常版versionCode 17をGoogle Play内部テストへアップロード・公開する操作は外部確定操作であり、別途アカウント所有者の明示確認を必要とする。
