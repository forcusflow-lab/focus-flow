...
| 項目 | 結果 | 根拠・次の扱い |
|---|---|---|
| Todo / Habit / Todayの必須Pill | 合格 | 実機画像で`必須`の全文表示を確認。v13でも回帰させない。 |
| Widgetの必須Pillと単一カード | 継続確認 | 実機画像で`必須`と単一カードは確認できるが、最終合格はサイズ追従と合わせて判定する。 |
| Widgetの拡大・縮小 | **不合格** | ランチャー上でサイズ変更に追従しないと利用者が報告。v12の`OPTION_APPWIDGET_SIZES`だけでは十分でない。 |

> v12のWidgetリサイズ受入は不合格である。v13では、resize callbackの実サイズをwidgetIdごとに保存し、通常更新・状態同期・操作後更新がその保存値を上書きしない構成へ変更してから再受入する。
