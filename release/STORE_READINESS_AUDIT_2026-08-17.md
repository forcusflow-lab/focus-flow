...
アプリの基本機能、端末内データ削除、通知、Androidの集中制限、プライバシー画面、サポート画面、iOS・Android共通のPlus購入・復元・管理導線は実装されています。一方で、公開・収益化の観点では、**公開URLとしてのプライバシーポリシー／連絡先未確定、ストアコンソールでの権限・データ利用・商品設定・実機確認の未完了**が残っています。

| 分類 | 現在の状態 | 対応方針 |
|---|---|---|
| Android課金 | `expo-iap`を通じたネイティブ購入・復元・管理導線と `focus_flow_plus` SKUを実装 | Play Consoleで商品、ベースプラン、ライセンステスターを設定し、内部テストで確認 |
| iOS課金 | `expo-iap`を通じたStoreKit対応の購入・復元・管理導線を実装 | App Store Connectでサブスクリプショングループ、商品、価格、サンドボックス／TestFlight確認を設定 |
...
Expoのアプリ内課金は、Expo Goではなくネイティブのストア向けビルドを必要とします。現行アプリはStoreKitとGoogle Play Billingを両対応するExpo向けIAPモジュールを使用し、物理端末・ストアのテスト商品で購入、復元、保留、更新、解約の各状態を検証します。[7] [8]
