...
## v13 実機受入結果（2026-08-27）

| 受入項目 | 結果 | 根拠・次の扱い |
|---|---|---|
| Widgetの拡大・縮小 | **合格** | 利用者から「拡大縮小は正しく機能しました」との報告を受領。添付画面で3件表示・カード外形の拡大状態を確認した。 |
| 3行表示 | **合格** | 未完了Todo、完了Todo、完了Habitの3件が同一Widget内へ表示された。 |
| 本体の必須Pill | 合格を維持 | v12実機で確認したTodo・Habit・Todayの`必須`全文表示に回帰は報告されていない。 |
| リサイズ後の同期・完了／復元 | 未確認 | アプリを開く、項目を更新する、Widgetで完了／復元する各操作後にも行数が維持されるかを確認する。 |
| ライト／ダークの最終可読性 | 未確認 | Widgetと本体の文字・Pill・チェックのコントラストを双方で確認する。 |

> Widgetリサイズに関するv12の不合格は、v13で実機合格へ更新した。未確認項目の受入が完了するまで、正式配布・Google Play配布は行わない。

## v13 追加実機フィードバック（2026-08-27）

Widgetの拡大・縮小は合格したが、時間型Habitの体験は**不合格**として追加記録する。Widgetでは時間型Habitがロックアイコン中心となり、計測状態や次の操作が分かりにくい。本体Todayでは`0/30分`に対して回数型と同じ±操作が表示され、チェック操作は完了状態変更ではなく詳細画面へ遷移するとの実機報告を受領した。

| 項目 | v13判定 | v14の必須受入条件 |
|---|---|---|
| Widgetリサイズ | 合格 | 継続維持 |
| 時間型Habitの本体操作 | 不合格 | 開始／一時停止、経過時間、完了可否を明示し、回数型の±を表示しない |
| 本体のチェック操作 | 不合格 | チェック領域は状態変更のみ、カード本文だけが詳細を開く |
| 時間型HabitのWidget表示 | 不合格 | 状態・残り時間・開始操作を本体と整合させ、ロックだけの曖昧な表示を廃止する |
| Widgetの視覚階層 | 再設計 | 本体と同じsurface／角丸／余白／タイポグラフィのフラットな一覧カードへ寄せる |
