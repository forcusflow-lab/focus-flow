...
| versionCode 8 統合ウィジェット版 | AAB生成済み・内部テスト追加待ち | GitHub Actions run `32514133323` が成功（25分17秒）。UX改善と統合Androidウィジェットを含む署名済みAABを生成し、Google Playアップロード証明書の検証を通過。成果物名は互換維持のため `focus-flow-v7-aab` と表示されるが、実行元コミットは `008ee0c`、Expo設定のversionCodeは8。次は内部テストの下書きリリースへ追加する。 |
