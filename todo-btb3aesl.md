...
- [x] UI/UX改善: Todoサブタスクの階層ツリー化、今日画面ウィジェットのTodo/習慣進捗識別、習慣編集画面のコンパクトなグループ化
...
- [ ] v27でToday残り件数を背景とのコントラストに応じた前景色・補助文・境界線へ調整し、6テーマ×light/darkで可読性を検証する
...
- [x] 最新コミット7343cd9の今回の修正をGitHub `main`へpushする（merge後main: d450418）
...
- [x] WidgetのTodo追加ボタンから背景・枠・角丸面を除去し、「今日の項目」と同じテーマprimary色のアイコン単体へ統一する
...
- [x] 添付ファイルの記載内容を確認し、対象機能と受入条件を確定する
- [x] 添付仕様に基づく実装・関連テスト・変更記録を完了する

- [x] 現在のUI改善変更を確認し、作業ブランチへcommit・pushする

- [x] 習慣カード本体のタップを詳細画面遷移へ戻し、Chevronだけで展開・折りたたみできるよう修正する
- [x] 習慣カードのタップ操作を最小限検証後、現在の作業ブランチへcommit・pushする

- [x] 添付ファイルの記載内容を確認し、対象範囲と受入条件を確定する
- [x] 添付仕様に基づく実装・必要な検証・Git操作を完了する

- [x] 最新コミットと一つ前のコミットの差分を確認し、今回のUI変更範囲を特定する
- [x] 最新UIレイアウト変更だけを安全にrevertし、指定検査後にmainへcommit・pushする

- [ ] Todoカードを指定の2行＋サブタスク折りたたみレイアウトへ変更し、文字切れを防止する
- [ ] Todo編集画面のサブタスク行をインライン編集可能にし、既存保存ボタンで保存できるようにする
- [ ] Todo修正を最小限確認し、mainへcommit・pushする

- [x] Todoカードを指定の2行レイアウトとサブタスク折りたたみに変更し、文字切れを防止する
- [x] Todo編集画面のサブタスク行をタップでインライン編集可能にし、既存保存ボタンで保存する
- [x] Todo限定改修を最小限確認し、mainへcommit・pushする

- [x] Todo・習慣・Today・Widgetのカード表示と必須ラベルを指定レイアウトへ統一する
- [x] 期限文言、不要な矢印・ゴミ箱、詳細画面の削除導線を修正する
- [x] 対象UIを最小限確認し、mainへcommit・pushする

- [x] Todo・今日・習慣・Widgetの必須表示をフルの「必須」ピル型へ共通化する
- [x] 期限表示をフルの「期限切れ」へ統一し、文字切れを防止する
- [x] 2点限定の変更を確認してmainへcommit・pushする

- [x] Todoタブ・今日タブ・習慣タブの必須表記とカード初期状態を統一する
- [x] Widgetの必須表示をアプリ内カードと同じ表記・見た目へ揃える
- [x] 変更範囲を最小限確認し、mainへcommit・pushする

- [x] Widgetが表示されない原因をRemoteViews経路・XML・drawable参照の整合性から特定する
- [x] 必須ラベルが全カードで「必須」と表示されることを確認し、必要な最小修正を行う
- [x] Widgetの表示可能性を構造検証し、型・リソース・関連テストを確認する
- [x] 修正をmainへcommit・pushする

- [x] 今日タブ・Todoタブ・習慣タブの必須表示を「必須」フル表記・共通ピル型・十分な幅へ揃える
- [x] Widget表示不能と4画面の必須表示を関連経路だけで検証し、mainへ修正を反映する
- [x] 今日タブ・Todoタブ・習慣タブ・Widgetの期限切れ表示をフル表記で確認・統一する
- [x] Todoカードの必須ピルが端末上で「必須」とフル表示される構造へ修正する
- [x] Todoカードのサブタスク初期状態を折りたたみに戻し、表示契約を再確認する（実機操作による展開であり、修正不要）
- [x] 修正をmainへcommit・pushする
- [x] Today・Todo・習慣・Widgetの必須ピル文字切れと表示崩れをコード側から再点検する
- [x] Widgetの必須ピルをTodayカードと同じくタイトル右側へ配置し、文字切れを防止する
- [x] 全画面・全状態・ネイティブ導線の監査基準と問題一覧を作成する
- [x] Focus Flowの正式仕様書・画面設計・状態遷移・受入基準を作成する
- [x] Todo・サブタスク・習慣・Today・進捗・設定・メモ・その他の問題を修正する
- [x] アプリ制限・Widget・Deep Link・戻る・起動・テーマ・文字倍率の問題を修正する
- [x] ユニット・統合・Android生成の検証を行い、修正と検証記録をGitHubへ反映する
- [x] GitHub Actions実行前の状態と実機で必要な受入項目を報告する
