...
private data class GateState(val active: Boolean, val rules: org.json.JSONArray?, val language: String) {
  fun ruleBlocking(packageName: String): GateRule? { if (!active || rules == null) return null; for (index in 0 until rules.length()) { val rule = rules.optJSONObject(index)?.let(::GateRule) ?: continue; if (rule.effectivePendingCount > 0 && rule.isWithinSchedule() && packageName in rule.blockedPackages) return rule }; return null }
}
private data class GateRule(val json: JSONObject) {
  ...
  val effectivePendingCount: Int get() {
    ...
    for (index in 0 until unlocks.length()) {
      val item = unlocks.optJSONObject(index) ?: continue
      if (item.optLong("endsAt") <= System.currentTimeMillis()) elapsed += 1
    }
