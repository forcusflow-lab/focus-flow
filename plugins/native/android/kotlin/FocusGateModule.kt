...
class FocusGateModule(private val context: ReactApplicationContext) : ReactContextBaseJavaModule(context) {
  ...
  @ReactMethod fun saveGateState(serialized: String, promise: Promise) {
    executor.execute {
      try {
        val preferences = context.getSharedPreferences(GATE_PREFS, Context.MODE_PRIVATE)
        if (preferences.getString(GATE_STATE, null) == serialized) {
        ...
        promise.reject("SAVE_GATE_STATE_FAILED", error)
      }
    }
  }
