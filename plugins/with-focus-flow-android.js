...
function withFocusFlowAndroid(config) {
  ...
  return withDangerousMod(config, ["android", async (config) => {
    ...
    const widgetItemDrawables = ["focus_flow_widget_item_light.xml", "focus_flow_widget_item_required_light.xml", "focus_flow_widget_item_done_light.xml", "focus_flow_widget_item_dark.xml", "focus_flow_widget_item_required_dark.xml", "focus_flow_widget_item_done_dark.xml", "focus_flow_widget_timer_start.xml", "focus_flow_widget_timer_pause_light.xml", "focus_flow_widget_timer_pause_dark.xml", "focus_flow_widget_pill_container.xml"].map((file) => ["drawable", file]);
    const widgetThemeSurfaces = ["mist", "slate", "evergreen", "ocean", "orchid", "sunrise"].flatMap((theme) => ["light", "dark"].map((appearance) => ["drawable", `focus_flow_widget_surface_${theme}_${appearance}.xml`]));
    const widgetBadgeDrawables = ["mist", "slate", "evergreen", "ocean", "orchid", "sunrise"].flatMap((theme) => ["light", "dark"].map((appearance) => ["drawable", `focus_flow_widget_badge_${theme}_${appearance}.xml`]));
    const widgetBadgeOpacityDrawables = ["mist", "slate", "evergreen", "ocean", "orchid", "sunrise"].flatMap((theme) => ["light", "dark"].flatMap((appearance) => [0, 25, 50, 75, 100].map((opacity) => ["drawable", `focus_flow_widget_badge_${theme}_${appearance}_${opacity}.xml`])));
    for (const [directory, file] of [["layout", "focus_flow_widget_initial.xml"], ["xml", "focus_flow_accessibility_service.xml"], ["xml", "focus_flow_widget_initial_info.xml"], ["values", "focus_flow_styles.xml"], ["values", "strings.xml"], ["drawable", "focus_flow_widget_background.xml"], ["drawable", "widget_bg_geometric.xml"], ["drawable", "widget_bg_geometric_dark.xml"], ["drawable", "widget_bg_aurora.xml"], ["drawable", "widget_bg_aurora_dark.xml"], ["drawable", "widget_bg_grid.xml"], ["drawable", "widget_bg_grid_dark.xml"], ["drawable", "focus_flow_widget_checkbox.xml"], ["drawable", "focus_flow_widget_checkbox_done.xml"], ["drawable", "focus_flow_widget_check_empty.xml"], ["drawable", "focus_flow_widget_check_locked.xml"], ["drawable", "focus_flow_widget_check_mark.xml"], ["drawable", "focus_flow_widget_badge_light.xml"], ["drawable", "focus_flow_widget_badge_dark.xml"], ["drawable", "focus_flow_widget_check_circle.xml"], ["drawable", "focus_flow_widget_check_circle_done.xml"], ["drawable", "focus_flow_widget_add_todo.xml"], ...widgetItemDrawables, ...widgetThemeSurfaces, ...widgetBadgeDrawables, ...widgetBadgeOpacityDrawables, ...cardDrawables, ...themeDrawables]) { const destination = path.join(resourceTarget, directory, file); fs.mkdirSync(path.dirname(destination, file), { recursive: true }); const resourceSource = fs.readFileSync(path.join(templateRoot, "res", directory, file), "utf8").replaceAll("$PACKAGE_NAME", packageName); fs.writeFileSync(destination, resourceSource); }
...
module.exports = createRunOncePlugin(withFocusFlowAndroid, PLUGIN_NAME, "1.0.18");
