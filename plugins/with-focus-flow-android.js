...
function withFocusFlowAndroid(config) {
  ...
  return withDangerousMod(config, ["android", async (config) => {
    ...
    for (const [directory, file] of [["layout", "focus_flow_widget.xml"], ["xml", "focus_flow_accessibility_service.xml"], ["xml", "focus_flow_widget_info.xml"], ["values", "focus_flow_styles.xml"], ["drawable", "focus_flow_widget_background.xml"], ["drawable", "focus_flow_widget_action_background.xml"], ["drawable", "focus_flow_widget_count_background.xml"], ["drawable", "focus_flow_widget_item_background.xml"], ["drawable", "focus_flow_widget_item_required.xml"], ["drawable", "focus_flow_widget_item_done.xml"], ...themeDrawables]) { const destination = path.join(resourceTarget, directory, file); fs.mkdirSync(path.dirname(destination), { recursive: true }); fs.copyFileSync(path.join(templateRoot, "res", directory, file), destination); }
