...
function withFocusFlowAndroid(config) {
  ...
  return withDangerousMod(config, ["android", async (config) => {
    ...
    for (const file of ["FocusGateActivity.kt", "FocusGateModule.kt", "FocusGatePackage.kt", "FocusGateService.kt", "FocusFlowWidgetProvider.kt", "FocusFlowWidgetItemsService.kt"]) fs.writeFileSync(path.join(kotlinTarget, file), fs.readFileSync(path.join(templateRoot, "kotlin", file), "utf8").replaceAll("$PACKAGE_NAME", packageName).replaceAll("$DEEP_LINK_SCHEME", deepLinkScheme));
    ...
    for (const [directory, file] of [["layout", "focus_flow_widget.xml"], ["layout", "focus_flow_widget_initial.xml"], ["layout", "focus_flow_widget_item.xml"], ["xml", "focus_flow_accessibility_service.xml"], ["xml", "focus_flow_widget_info.xml"], ["xml", "focus_flow_widget_initial_info.xml"], ["values", "focus_flow_styles.xml"], ["drawable", "focus_flow_widget_background.xml"], ["drawable", "focus_flow_widget_action_background.xml"], ["drawable", "focus_flow_widget_count_background.xml"], ["drawable", "focus_flow_widget_checkbox.xml"], ["drawable", "focus_flow_widget_checkbox_done.xml"], ["drawable", "focus_flow_widget_check_empty.xml"], ["drawable", "focus_flow_widget_check_locked.xml"], ["drawable", "focus_flow_widget_check_mark.xml"], ["drawable", "focus_flow_widget_item_background.xml"], ["drawable", "focus_flow_widget_item_required.xml"], ["drawable", "focus_flow_widget_item_done.xml"], ["drawable", "focus_flow_widget_item_light.xml"], ["drawable", "focus_flow_widget_item_dark.xml"], ["drawable", "focus_flow_widget_item_done_light.xml"], ["drawable", "focus_flow_widget_item_done_dark.xml"], ...themeDrawables]) { const destination = path.join(resourceTarget, directory, file); fs.mkdirSync(path.dirname(destination, file), { recursive: true }); fs.copyFileSync(path.join(templateRoot, "res", directory, file), destination); }
