...
function withFocusFlowAndroid(config) {
  ...
  return withDangerousMod(config, ["android", async (config) => {
    ...
    for (const file of ["FocusGateActivity.kt", "FocusGateModule.kt", "FocusGatePackage.kt", "FocusGateService.kt", "FocusFlowWidgetProvider.kt", "FocusFlowWidgetTextView.kt"]) fs.writeFileSync(path.join(kotlinTarget, file), fs.readFileSync(path.join(templateRoot, "kotlin", file), "utf8").replaceAll("$PACKAGE_NAME", packageName).replaceAll("$DEEP_LINK_SCHEME", deepLinkScheme));
