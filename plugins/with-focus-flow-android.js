...
function withFocusFlowAndroid(config) {
  ...
  return withDangerousMod(config, ["android", async (config) => {
    ...
    for (const [directory, file] of [["layout", "focus_flow_widget_initial.xml"], ["xml", "focus_flow_accessibility_service.xml"], ["xml", "focus_flow_widget_initial_info.xml"], ["values", "focus_flow_styles.xml"], ["drawable", "focus_flow_widget_background.xml"], ["drawable", "focus_flow_widget_checkbox.xml"], ["drawable", "focus_flow_widget_check_empty.xml"], ["drawable", "focus_flow_widget_check_locked.xml"], ["drawable", "focus_flow_widget_check_mark.xml"], ...themeDrawables]) { const destination = path.join(resourceTarget, directory, file); fs.mkdirSync(path.dirname(destination, file), { recursive: true }); fs.copyFileSync(path.join(templateRoot, "res", directory, file), destination); }
...
module.exports = createRunOncePlugin(withFocusFlowAndroid, PLUGIN_NAME, "1.0.7");
