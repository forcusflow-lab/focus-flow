...
2. **完了チェック**:
   - Todoカードと完全同一の外寸24dp（角丸6dp、枠線1.5dp、アイコン15dp）。44dpのタッチターゲット（`checkTouchTarget`）を確保し、左側に配置。
3. **タイトル・フォント**:
   - Todoカードと同一のフォントサイズ（14dp）、行高（19dp）、太字（`800`）に統一。
4. **メタ情報**:
   - 必須バッジ（`Pill`）を最優先で左端に配置し、進捗ピル（例: `今日 0/10回`、背景色: `habit.color + "1A"`）および週間記録テキストを同一のメタ行（`minHeight: 19dp`, `gap: 5dp`）で統一。
5. **展開ボタン（共通仕様）**:
   - 閉じている状態: ∨ (`keyboard-arrow-down`)、開いている状態: ⌃ (`keyboard-arrow-up`)。
   - タイトル行右端に統一配置（サイズ32dp × 32dp、アイコン19dp、hitSlop 8dp）。
6. **展開エリアの構造（共通仕様）**:
   - 「親カードヘッダー → 共通の区切り線（`cardDivider`） → 展開コンテンツ（曜日・進捗操作）」の階層構造をTodoサブタスクと統一。二重枠線の不要な別カード化を排除。
7. **展開操作の分離（再発防止最重要ルール）**:
