...
export default function TodosScreen() {
  ...
  <TaskForm visible={formOpen} todo={editingTodo} onClose={() => { setFormOpen(false); setEditingTodo(undefined); if (widgetOpenedTodo.current) { widgetOpenedTodo.current = undefined; router.replace("/(tabs)" as never); } }} onSave={(input) => { const result = editingTodo ? updateTodo(editingTodo.id, input) : addTodo(input); showMutationResult(result); return result; }} />
  </ScreenContainer>;
}
