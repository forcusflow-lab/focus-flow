...
export default function InsightsScreen() {
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><FlatList data={completed} keyExtractor={(item) => item.key} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}
    ListHeaderComponent={<><ScreenHeading eyebrow={t("続ける仕組みを整える", "Strengthen your system")} title={t("振り返り", "Review")} /><View style={styles.summaryRow}><Metric label={t("完了Todo", "Tasks done")} value={`${weekTodoCount}`} detail={t("過去7日間", "Last 7 days")} color={COLORS.forest} /><Metric label={t("習慣", "Habits")} value={habitTargets ? `${habitCompletions}/${habitTargets}` : "—"} detail={t("今週の記録", "This week")} color={COLORS.warning} /><Metric label={t("残り", "Left")} value={`${gate.pendingCount}`} detail={t("今の解除条件", "Current must-dos")} color={gate.pendingCount ? COLORS.error : COLORS.success} /></View><View style={[styles.ruleCard, activeWindow || hasAlwaysRule ? styles.ruleCardActive : styles.ruleCardPaused]}><Text style={styles.ruleTitle}>{ruleTitle}</Text><Text style={styles.ruleText}>{ruleText}</Text></View><Text style={styles.sectionTitle}>{t("今週の完了Todo", "Completed tasks this week")}</Text></>}
