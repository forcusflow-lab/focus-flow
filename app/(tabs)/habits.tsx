...
export default function HabitsScreen() {
  ...
  /><HabitForm visible={formOpen} habit={editingHabit} defaultRequired={newHabitDefaultRequired} onClose={() => { setFormOpen(false); setEditingHabit(undefined); setNewHabitDefaultRequired(false); router.setParams({ open: undefined }); if (widgetOpenedHabit.current) { widgetOpenedHabit.current = undefined; router.replace("/(tabs)" as never); } }} onSave={(input) => { const result = editingHabit ? updateHabit(editingHabit.id, input) : addHabit(input); showMutationResult(result); return result; }} onDelete={editingHabit ? () => deleteHabit(editingHabit.id) : undefined} /></ScreenContainer>;
}

const styles = StyleSheet.create({ content: { paddingTop: 10, paddingBottom: 20, flexGrow: 1 }, summary: { borderWidth: 1, borderRadius: 18, paddingHorizontal: 14, paddingTop: 11, paddingBottom: 11, marginTop: -4, marginBottom: 10 }, summaryTop: { flexDirection: "row", alignItems: "center", gap: 10 }, summaryIcon: { width: 34, height: 34, borderRadius: 10, alignItems: "center", justifyContent: "center" }, summaryCopy: { flex: 1, minWidth: 0 }, summaryEyebrow: { fontSize: 9.5, letterSpacing: 0.5, fontWeight: "900" }, summaryTitle: { fontSize: 13, lineHeight: 18, fontWeight: "800", marginTop: 1 }, summaryPercent: { fontSize: 20, fontWeight: "900" }, progressTrack: { height: 6, borderRadius: 999, overflow: "hidden", marginTop: 9 }, progressFill: { height: "100%", borderRadius: 999 }, summaryHint: { fontSize: 10.5, lineHeight: 14, marginTop: 7 }, groupHeading: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 4, marginBottom: 7 }, groupTitle: { flex: 1, minWidth: 0, paddingRight: 10, fontSize: 15, lineHeight: 21, fontWeight: "900" }, groupCount: { minWidth: 25, height: 25, textAlign: "center", textAlignVertical: "center", overflow: "hidden", borderRadius: 13, fontSize: 12, fontWeight: "800" }, revealButton: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 14, borderWidth: 1, marginTop: 8 }, revealText: { fontSize: 13, fontWeight: "800" } });
