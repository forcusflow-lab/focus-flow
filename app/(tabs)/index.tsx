...
export default function TodayScreen() {
  ...
  useEffect(() => { const reveal = Array.isArray(params.completed) ? params.completed[0] : params.completed; if (reveal === "1") setShowHiddenCompleted(true); }, [params.completed]);
  ...
  const allCompletedTodos = useMemo(() => effectiveTodos.filter((todo) => isTodoAchieved(todo)), [effectiveTodos]);
  const allCompletedHabits = useMemo(() => effectiveHabits.filter((habit) => isHabitCompleteOn(habit, today)), [effectiveHabits, today]);
  const completedTodos = useMemo(() => todayCompletedDisplay === "dim" || showHiddenCompleted ? allCompletedTodos : [], [allCompletedTodos, showHiddenCompleted, todayCompletedDisplay]);
  const completedHabits = useMemo(() => todayCompletedDisplay === "dim" || showHiddenCompleted ? allCompletedHabits : [], [allCompletedHabits, showHiddenCompleted, todayCompletedDisplay]);
  const hiddenCompletedCount = allCompletedTodos.length + allCompletedHabits.length;
  ...
  const explainTimedResult = (reason: string | undefined, destination: "/(tabs)/todos" | "/(tabs)/habits") => {
    if (reason === "TIMER_ALREADY_RUNNING") Alert.alert(t("すでに計測中です", "Timer is already running"), t("停止すると、同じ場所から再開できます。", "Pause it to resume from the same elapsed time."), [{ text: t("確認", "OK") }]);
  };
  ...
  const handleHabitToggle = (id: string, date?: string) => { const result = toggleHabit(id, date); safeHaptic(result.ok ? "success" : "light"); explainTimedResult(result.reason, "/(tabs)/habits"); };
...
function HomeHabitCard({ habit, required, windowLabel, english, onToggle, onStartTimer, onPauseTimer, onProgress, onOpen }: { habit: Habit; required: boolean; windowLabel?: string; english: boolean; onToggle: (date?: string) => void; onStartTimer: () => void; onPauseTimer: () => void; onProgress: (delta: number) => void; onOpen: () => void }) {
  const palette = useFocusPalette(); const today = dayKey(); const language = english ? "en" : "ja"; const done = isHabitCompleteOn(habit, today); const weekly = weeklyHabitProgress(habit); const week = Array.from({ length: 7 }, (_, index) => dayKeyOffset(index - 6));
  const t = (ja: string, en: string) => english ? en : ja;
  return <View style={[styles.todayHabitRow, { backgroundColor: done ? palette.elevated : required ? palette.primarySoft : palette.surface, borderColor: palette.border }]}><View style={[styles.itemRail, { backgroundColor: habit.color }]} /><TouchableOpacity accessibilityRole="checkbox" accessibilityState={{ checked: done }} accessibilityLabel={t(`「${habit.title}」を今日の習慣として記録`, `Record “${habit.title}” for today`)} onPressIn={(event) => event.stopPropagation()} onPress={() => onToggle()} style={styles.itemCheckTouchTarget}><View style={[styles.itemCheck, { borderColor: palette.border }, done && { backgroundColor: habit.color, borderColor: habit.color }]}>{done ? <MaterialIcons name="check" size={17} color={COLORS.white} /> : null}</View></TouchableOpacity><View style={styles.todayHabitCopy}><TouchableOpacity accessibilityRole="button" onPress={onOpen}><Text style={[styles.itemTitle, done && styles.todayHabitTitleDone]} numberOfLines={1}>{habit.title}</Text><View style={styles.meta}>{required ? <View style={styles.requiredPillSlot}><Pill label={t("必須", "Must-do")} color={COLORS.forest} /></View> : null}<Text style={styles.itemMeta}>{windowLabel ? `${windowLabel} · ` : ""}{t(`週 ${weekly.completed}/${weekly.target}`, `${weekly.completed}/${weekly.target} this week`)} · {habitStreak(habit)}{t("日連続", "-day streak")}</Text></View></TouchableOpacity><View style={styles.todayWeekRow}>{week.map((key) => { const marked = isHabitCompleteOn(habit, key); const current = key === today; return <TouchableOpacity key={key} accessibilityLabel={t(`${shortWeekday(key)}曜日を記録`, `Record ${shortWeekday(key, language)}`)} hitSlop={7} onPress={() => onToggle(key)} style={[styles.todayDayDot, { borderColor: palette.border }, current && { borderColor: habit.color }, marked && { backgroundColor: habit.color, borderColor: habit.color }]}>{marked ? <MaterialIcons name="check" size={10} color={COLORS.white} /> : <Text style={[styles.todayDayLetter, current && { color: habit.color }]}>{shortWeekday(key, language)}</Text>}</TouchableOpacity>})}</View><HabitProgressControl habit={habit} date={today} language={language} onAdjust={onProgress} onStartTimer={onStartTimer} onPauseTimer={onPauseTimer} /></View><TouchableOpacity accessibilityLabel={t("習慣の詳細を開く", "Open habit details")} hitSlop={8} onPress={onOpen} style={styles.openControl}><MaterialIcons name="chevron-right" size={21} color={palette.muted} /></TouchableOpacity></View>;
}
...
const styles = StyleSheet.create({
  ...
  itemCard: { position: "relative", minHeight: 68, flexDirection: "row", alignItems: "center", overflow: "hidden", backgroundColor: "rgba(255,255,255,0.92)", borderWidth: 1, borderColor: COLORS.border, borderRadius: 15, paddingVertical: 8, paddingLeft: 14, paddingRight: 4, marginBottom: 6 }, requiredItemCard: { backgroundColor: "#F4FBF7", borderColor: "#B9DDD1" }, itemRail: { position: "absolute", left: 0, top: 0, bottom: 0, width: 4 }, itemCheckTouchTarget: { width: 44, height: 44, alignItems: "center", justifyContent: "center", marginLeft: -8, marginRight: 2 }, itemCheck: { width: 28, height: 28, borderRadius: 6, borderWidth: 1.5, borderColor: "#AFC0B7", alignItems: "center", justifyContent: "center" }, itemCopy: { flex: 1, minWidth: 0 }, habitTitleRow: { flexDirection: "row", alignItems: "center", gap: 7 }, habitDot: { width: 8, height: 8, borderRadius: 4, flexShrink: 0 }, itemTitle: { color: COLORS.text, fontSize: 15, lineHeight: 21, fontWeight: "800" }, meta: { width: "100%", minHeight: 52, flexDirection: "column", alignItems: "flex-start", gap: 4, marginTop: 4, overflow: "visible" }, requiredPillSlot: { minWidth: 84, minHeight: 34, alignSelf: "stretch", alignItems: "flex-start", justifyContent: "center", overflow: "visible" }, itemMeta: { width: "100%", color: COLORS.muted, fontSize: 11, lineHeight: 16, fontWeight: "700" }, windowMeta: { width: "100%", color: "#7A5A22", fontSize: 11, lineHeight: 16, fontWeight: "800" }, overdue: { color: COLORS.error }, openControl: { width: 34, height: 40, alignItems: "center", justifyContent: "center", marginLeft: 2 }, todayHabitRow: { position: "relative", minHeight: 102, flexDirection: "row", alignItems: "flex-start", overflow: "hidden", borderWidth: 1, borderRadius: 15, paddingVertical: 9, paddingLeft: 14, paddingRight: 4, marginBottom: 6 }, todayHabitCopy: { flex: 1, minWidth: 0, overflow: "visible" }, todayHabitTitleDone: { color: "#75827C", textDecorationLine: "line-through", textDecorationColor: "#75827C" }, todayWeekRow: { flexDirection: "row", gap: 6, marginTop: 6 }, todayDayDot: { width: 22, height: 22, borderRadius: 11, borderWidth: 1, alignItems: "center", justifyContent: "center" }, todayDayLetter: { color: COLORS.muted, fontSize: 9, fontWeight: "800" }, todayProgressControl: { alignSelf: "flex-start", flexDirection: "row", alignItems: "center", gap: 7, borderRadius: 10, marginTop: 6, paddingHorizontal: 5, paddingVertical: 3 }, todayProgressButton: { width: 25, height: 25, alignItems: "center", justifyContent: "center", borderRadius: 7 }, todayProgressText: { minWidth: 48, textAlign: "center", fontSize: 11, fontWeight: "900" },
  ...
  revealCompleted: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 14, borderWidth: 1, marginTop: 10 }, revealCompletedText: { fontSize: 13, fontWeight: "800" },
});
