...
export default function TodayScreen() {
  ...
  const { todos, habits, memos, focusSessions, gateConfig, displaySettings, isReady, toggleTodo, toggleHabit, startHabitTimer, adjustHabitProgress, addTodo, updateTodo, updateHabit } = useFocusFlow();
  ...
  const explainTimedResult = (reason: string | undefined, destination: "/(tabs)/todos" | "/(tabs)/habits") => {
    ...
    if (reason === "TIMER_NOT_STARTED") Alert.alert(t("まず計測を開始してください", "Start the timer first"), t("時間型の習慣は「開始」を押してから、設定時間を計測します。", "For a timed habit, press Start before the target time is counted."), [{ text: t("確認", "OK") }]);
...
function HomeHabitCard({ habit, required, windowLabel, english, onToggle, onStartTimer, onProgress, onOpen }: { habit: Habit; required: boolean; windowLabel?: string; english: boolean; onToggle: (date?: string) => void; onStartTimer: () => void; onProgress: (delta: number) => void; onOpen: () => void }) {
