...
export default function TodayScreen() {
  ...
  const listItems = useMemo<HomeListItem[]>(() => {
    ...
    if (regular.length) result.push({ type: "heading", id: "today-heading", title: t("今日の項目", "Today’s items"), detail: t("Todoと習慣", "Tasks and habits"), required: false }, ...regular);
    return result;
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background">
    <FlatList
      ...
      renderItem={({ item }) => item.type === "heading" ? <View style={[styles.sectionHeader, item.required && styles.requiredSectionHeader]}><Text style={[styles.sectionTitle, item.required && styles.requiredSectionTitle]}>{item.title}</Text><Text style={[styles.sectionHint, item.required && styles.requiredSectionHint]}>{item.detail}</Text></View> : item.type === "todo" ? <HomeTodoCard todo={item.todo} required={item.required} english={english} onToggle={() => handleTodoToggle(item.todo.id)} onOpen={() => router.push("/(tabs)/todos")} /> : <HomeHabitCard habit={item.habit} required={item.required} english={english} onToggle={() => handleHabitToggle(item.habit.id)} onOpen={() => router.push("/(tabs)/habits")} />}
    />
...
const styles = StyleSheet.create({
  ...
  itemCard: { position: "relative", minHeight: 74, flexDirection: "row", alignItems: "center", overflow: "hidden", backgroundColor: "rgba(255,255,255,0.92)", borderWidth: 1, borderColor: COLORS.border, borderRadius: 17, paddingVertical: 12, paddingLeft: 14, paddingRight: 4, marginBottom: 8 }, requiredItemCard: { backgroundColor: "#F4FBF7", borderColor: "#B9DDD1" }, itemRail: { position: "absolute", left: 0, top: 0, bottom: 0, width: 4 }, itemCheck: { width: 28, height: 28, borderRadius: 10, borderWidth: 1.5, borderColor: "#AFC0B7", marginRight: 10 }, itemCopy: { flex: 1, minWidth: 0 }, habitTitleRow: { flexDirection: "row", alignItems: "center", gap: 7 }, habitDot: { width: 8, height: 8, borderRadius: 4, flexShrink: 0 }, itemTitle: { color: COLORS.text, fontSize: 15, lineHeight: 21, fontWeight: "800" }, meta: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", gap: 6, marginTop: 6 }, itemMeta: { color: COLORS.muted, fontSize: 11, lineHeight: 16, fontWeight: "700" }, overdue: { color: COLORS.error }, openControl: { width: 34, height: 40, alignItems: "center", justifyContent: "center", marginLeft: 2 },
