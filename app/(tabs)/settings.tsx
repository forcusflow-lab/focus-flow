        ...
        ListHeaderComponent={
          <>
            ...
            <View style={styles.card}>
              ...
              {!isPlus ? <View style={styles.plusActionArea}>
                {plusStatus.status === "eligible" ? <><TouchableOpacity accessibilityRole="button" onPress={() => void purchasePlus()} style={styles.plusPrimary}><Text style={styles.plusPrimaryText}>{plusStatus.price ? t(`${plusStatus.price} で Plus を開始`, `Get Plus for ${plusStatus.price}`) : t("Plus を開始", "Get Plus")}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" onPress={() => void restorePlus()} style={styles.plusSecondary}><Text style={styles.plusSecondaryText}>{t("以前の購入を復元", "Restore previous purchase")}</Text></TouchableOpacity></> : plusStatus.status === "loading" || plusStatus.status === "pending" ? <View style={styles.plusBusy}><ActivityIndicator color={COLORS.forest} /><Text style={styles.plusBusyText}>{plusStatus.status === "pending" ? t("ストアで購入を完了し、アプリへ戻ってください。", "Finish the purchase in your store, then return here.") : t("ストアの商品を確認しています…", "Checking store products…")}</Text></View> : plusStatus.reason === "NATIVE_BUILD_REQUIRED" ? <View style={styles.storeBuildNotice}><View style={styles.storeBuildNoticeHeader}><MaterialIcons name={isIOS ? "phone-iphone" : "android"} size={20} color="#215B83" /><Text style={styles.storeBuildNoticeTitle}>{isIOS ? t("App Store版で購入できます", "Available in the App Store build") : t("Google Play版で購入できます", "Available in the Google Play build")}</Text></View><Text style={styles.storeBuildNoticeText}>{isIOS ? t("このExpo Goプレビューでは購入や復元はできません。App Storeから入れたFocus Flowで、Plusの開始・購入の復元・管理を行えます。", "Purchases and restores are not available in this Expo Go preview. Use Focus Flow installed from the App Store to start, restore, or manage Plus.") : t("このExpo Goプレビューでは購入や復元はできません。Google Playから入れたFocus Flowで、Plusの開始・購入の復元・管理を行えます。", "Purchases and restores are not available in this Expo Go preview. Use Focus Flow installed from Google Play to start, restore, or manage Plus.")}</Text></View> : <View style={styles.storeIssueNotice}><Text style={styles.storeIssueNoticeText}>{plusStatus.status === "error" ? t("ストアに接続できません。ネットワークを確認してから、もう一度お試しください。", "The store could not be reached. Check your connection and try again.") : t("このストア用ビルドではPlusの商品がまだ設定されていません。Play ConsoleまたはApp Store Connectでテスト商品を設定すると、ここに実際の価格が表示されます。", "Plus is not configured for this store build. Set up a test product in Play Console or App Store Connect to show the live price here.")}</Text><TouchableOpacity accessibilityRole="button" onPress={() => void refreshPlusStatus()} style={styles.plusSecondary}><Text style={styles.plusSecondaryText}>{t("ストア情報を再確認", "Check store status")}</Text></TouchableOpacity></View>}
...
Object.assign(styles, StyleSheet.create({
  ...
  storeBuildNotice: { gap: 8, padding: 14, borderRadius: 15, borderWidth: 1, borderColor: "#B8D5E7", backgroundColor: "#EAF4FA" },
  storeBuildNoticeHeader: { flexDirection: "row", gap: 8, alignItems: "center" },
  storeBuildNoticeTitle: { flex: 1, minWidth: 0, color: "#173B59", fontSize: 14, lineHeight: 20, fontWeight: "900" },
  storeBuildNoticeText: { color: "#3D6077", fontSize: 12, lineHeight: 18 },
  storeIssueNotice: { gap: 10, padding: 13, borderRadius: 14, backgroundColor: "#FFF6E7", borderWidth: 1, borderColor: "#F0CF92" },
  storeIssueNoticeText: { color: "#73531B", fontSize: 12, lineHeight: 18, fontWeight: "700" },
