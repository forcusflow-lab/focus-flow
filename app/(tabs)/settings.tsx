...
export default function SettingsScreen() {
  ...
  const params = useLocalSearchParams<{ panel?: SettingsPanel }>();
  ...
  useEffect(() => {
    if (params.panel && ["limits", "appearance", "reminders", "plus", "home"].includes(params.panel)) {
      setPanel(params.panel);
      if (params.panel === "plus" && !isPlus) {
        setPaywallOpen(true);
      }
    }
  }, [params.panel, isPlus]);
  ...
  const selectGlobalApp = (packageName: string) => {
    ...
    if (!selected && !canSelectBlockedApp(packageName)) {
      Alert.alert(
        ...
        t("無料版では制限対象アプリを5件まで選べます。Plusに登録すると、アプリを無制限に制限でき、集中を深められます。", "The free plan allows 5 limited apps. Plus allows unlimited app limits to deepen your focus."),
        [
          { text: t("閉じる", "Dismiss"), style: "cancel" },
          { text: t("Plusを確認", "View Plus"), onPress: () => setPanel("plus") },
        ]
      );
      return;
    }
  ...
  const body = panel === "home" ? (
    ...
    <ScrollView key={panel} contentContainerStyle={[styles.detailContent, { paddingBottom: Math.max(88, insets.bottom + 44) }]} showsVerticalScrollIndicator={false}>
      ...
      {panel === "appearance" ? <><AppearancePanel english={english} displaySettings={displaySettings} isPlus={isPlus} onOpenPlus={() => { setPaywallOpen(true); setPanel("plus"); }} onChange={setDisplaySettings} /><WidgetsPanel english={english} displaySettings={displaySettings} isPlus={isPlus} onOpenPlus={() => { setPaywallOpen(true); setPanel("plus"); }} onBackgroundOpacity={(widgetBackgroundOpacity) => setDisplaySettings({ widgetBackgroundOpacity })} onCardOpacity={(widgetCardOpacity) => setDisplaySettings({ widgetCardOpacity })} onBackgroundStyle={(widgetBackgroundStyle) => setDisplaySettings({ widgetBackgroundStyle })} /></> : null}
...
function LimitsPanel({ english, isIOS, nativeReady, loadingApps, apps, appPickerOpen, onToggleAppPicker, selectedApps, onSelectApp, enabled, strictMode, pendingCount, scheduleActive, accessibilityEnabled, diagnostics, schedules, routineOpenId, onToggleEnabled, onToggleStrictMode, onOpenAccessibility, onRequestBatteryOptimization, onRefresh, onOpenAppInfo, onAddSchedule, onToggleRoutine, onUpdateSchedule, onRemoveSchedule, canSelectBlockedApp }: { english: boolean; isIOS: boolean; nativeReady: boolean; loadingApps: boolean; apps: LaunchableApp[]; appPickerOpen: boolean; onToggleAppPicker: () => void; selectedApps: string[]; onSelectApp: (value: string) => void; enabled: boolean; strictMode: boolean; pendingCount: number; scheduleActive: boolean; accessibilityEnabled: boolean; diagnostics?: GateDiagnostics; schedules: GateSchedule[]; routineOpenId?: string; onToggleEnabled: (value: boolean) => void; onToggleStrictMode: (value: boolean) => void; onOpenAccessibility: () => void; onRequestBatteryOptimization?: () => void; onRefresh: () => void; onOpenAppInfo: () => void; onAddSchedule: () => void; onToggleRoutine: (id?: string) => void; onUpdateSchedule: (id: string, input: Partial<GateSchedule>) => void; onRemoveSchedule: (id: string) => void; canSelectBlockedApp: (packageName: string) => boolean }) {
  ...
  return <>
    ...
    <InfoCard icon="format-list-numbered" title={t("設定は3つの順番で進めます", "Set up in three steps")} detail={t("1. 権限許可 → 2. アプリ選択 → 3. 時間帯設定。時間帯を選ばない場合はいつでも有効です。", "1. Permission → 2. Choose apps → 3. Time windows. Without a window, limits apply anytime.")} />
    <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.border }]}><SettingRow title={t("タスク完了までアプリを制限", "Limit apps until tasks are done")} detail={t("オンにする前に、Androidのアクセシビリティ許可を確認します。", "Android Accessibility permission is checked before enabling.")} control={<Switch value={enabled} onValueChange={onToggleEnabled} trackColor={{ false: "#CCD7D1", true: "#91C3B3" }} thumbColor={enabled ? COLORS.forest : "#F7F8F5"} />} /><SettingRow title={t("厳格モード", "Strict mode")} detail={strictMode ? t("未完了の必須項目がある間は、ここから集中制限をオフにできません。遮断画面からアプリ情報も開きません。", "While must-dos remain, App limits can't be turned off here and the gate doesn't link to app info.") : t("制限中の設定変更やアプリ削除を防止し、うっかり解除を防ぎます。", "Prevents changing settings or deleting apps while limits are active.")} control={<Switch value={strictMode} onValueChange={onToggleStrictMode} trackColor={{ false: "#CCD7D1", true: "#BE94E1" }} thumbColor={strictMode ? "#74509B" : "#F7F8F5"} />} /></View>
    <SectionTitle title={t("1. Androidの許可と動作確認", "1. Android permission & status")} />
    <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.border }]}>
      <SettingRow
        title={accessibilityEnabled ? t("アクセシビリティは有効です", "Accessibility is enabled") : t("アクセシビリティを許可する", "Allow Accessibility")}
        detail={accessibilityEnabled ? t("選択アプリの前面化を検出できます。", "Focus Flow can detect selected apps in the foreground.") : t("許可されない限り、アプリ制限は動作しません。", "App limits cannot run until this is enabled.")}
        control={
      ...
      />
      <View style={[styles.compactStatus, { backgroundColor: palette.elevated }]}>
        <MaterialIcons name={diagnostics?.backgroundRestricted ? "warning-amber" : "check-circle"} size={16} color={diagnostics?.backgroundRestricted ? COLORS.warning : COLORS.success} />
        <Text style={[styles.compactStatusText, { color: palette.text }]}>{diagnostics?.backgroundRestricted ? t("バックグラウンド実行が制限されています。端末のアプリ情報で許可してください。", "Background activity is restricted. Allow it in app info.") : t("バックグラウンド実行に大きな制限は検出されませんでした。", "No major background restriction was detected.")}</Text>
      </View>
      <View style={[styles.compactStatus, { backgroundColor: palette.elevated }]}>
        <MaterialIcons name={diagnostics?.lastGateEventAt ? "visibility" : "sync-problem"} size={16} color={diagnostics?.lastGateEventAt ? COLORS.success : COLORS.warning} />
        <Text style={[styles.compactStatusText, { color: palette.text }]}>{runtimeState}{"\n"}{eventState}</Text>
      </View>
      <View style={styles.inlineActions}>
        <TouchableOpacity onPress={onRefresh} style={styles.textAction}><Text style={[styles.textActionLabel, { color: palette.primary }]}>{t("状態を再確認", "Refresh status")}</Text></TouchableOpacity>
        <TouchableOpacity onPress={onOpenAppInfo} style={styles.textAction}><Text style={[styles.textActionLabel, { color: palette.primary }]}>{t("アプリ情報", "App info")}</Text></TouchableOpacity>
      </View>
    </View>
    ...
    <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.border }]}>
      <TouchableOpacity onPress={onToggleAppPicker} style={styles.selectorRow}>
        <View style={[styles.selectorIcon, { backgroundColor: palette.primarySoft }]}><MaterialIcons name="apps" size={20} color={palette.primary} /></View>
        <View style={styles.selectorCopy}>
          <Text style={[styles.selectorTitle, { color: palette.text }]}>{selectedApps.length ? t(`${selectedApps.length}件を選択中`, `${selectedApps.length} selected`) : t("アプリを選ぶ", "Choose apps")}</Text>
          <Text style={[styles.selectorDetail, { color: palette.muted }]}>{t("銀行・決済・連絡・地図アプリは選ばないでください。", "Do not select banking, payment, messaging, or navigation apps.")}</Text>
        </View>
        <MaterialIcons name={appPickerOpen ? "expand-less" : "chevron-right"} size={22} color={palette.muted} />
      </TouchableOpacity>
...
function AppearancePanel({ english, displaySettings, onChange, isPlus, onOpenPlus }: { english: boolean; displaySettings: DisplaySettings; onChange: (input: Partial<DisplaySettings>) => void; isPlus?: boolean; onOpenPlus?: () => void }) {
  ...
  return <>
    ...
    <View style={appearanceStyles.fontList}>
      {APP_FONT_OPTIONS.map((item) => (
        <FontChoice
          key={item.id}
          item={item}
          selected={(displaySettings.fontFamily ?? "system") === item.id}
          english={english}
          palette={palette}
          isPlus={Boolean(isPlus)}
          onPress={() => {
            if (item.id !== "system" && !isPlus) {
              Alert.alert(
                t("Plus限定の機能です", "Plus Feature"),
                t(
                  "プレミアムフォント（リーディング・ノート・フォーカス）はPlus限定です。Plusに登録して文字をカスタマイズしますか？",
                  "Premium typography (Reading, Notebook, Focus) is available with Plus. Upgrade to customize your fonts?"
                ),
                [
...
function OpacitySlider({ english, label, detail, value, onChange, onDrag, isPlus = true, onOpenPlus }: { english: boolean; label: string; detail: string; value: number; onChange: (value: number) => void; onDrag?: (value: number | undefined) => void; isPlus?: boolean; onOpenPlus?: () => void }) {
  const palette = useFocusPalette();
  ...
  const onOpenPlusRef = useRef(onOpenPlus);
  onOpenPlusRef.current = onOpenPlus;
  ...
  const responder = useMemo(() => PanResponder.create({
    ...
    onPanResponderGrant: (event) => {
      if (isPlusRef.current === false && onOpenPlusRef.current) {
        onOpenPlusRef.current();
        return;
      }
...
function WidgetsPanel({ english, displaySettings, isPlus, onOpenPlus, onBackgroundOpacity, onCardOpacity, onBackgroundStyle }: { english: boolean; displaySettings: DisplaySettings; isPlus?: boolean; onOpenPlus?: () => void; onBackgroundOpacity: (value: number) => void; onCardOpacity: (value: number) => void; onBackgroundStyle: (value: WidgetBackgroundStyle) => void }) {
  ...
  const bgStyle = displaySettings.widgetBackgroundStyle ?? "solid";
  const [dragBgOpacity, setDragBgOpacity] = useState<number>();
  const [dragCardOpacity, setDragCardOpacity] = useState<number>();
  const [committedBgOpacity, setCommittedBgOpacity] = useState<number>();
  const [committedCardOpacity, setCommittedCardOpacity] = useState<number>();
  const [localBgStyle, setLocalBgStyle] = useState<WidgetBackgroundStyle>(bgStyle);
  useEffect(() => { setLocalBgStyle(bgStyle); }, [bgStyle]);
  useEffect(() => { setCommittedBgOpacity(undefined); }, [bgValue]);
  useEffect(() => { setCommittedCardOpacity(undefined); }, [cardValue]);
  const previewBg = dragBgOpacity ?? committedBgOpacity ?? bgValue;
  const previewCard = dragCardOpacity ?? committedCardOpacity ?? cardValue;
  ...
  return <>
    ...
    <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.border }]}><View style={appearanceStyles.widgetFollowRow}><View style={[styles.selectorIcon, { backgroundColor: palette.primarySoft }]}><MaterialIcons name="palette" size={20} color={palette.primary} /></View><View style={styles.selectorCopy}><Text style={[styles.selectorTitle, { color: palette.text }]}>{t("本体のテーマを使用", "Follow app theme")}</Text><Text style={[styles.selectorDetail, { color: palette.muted }]}>{english ? APP_THEMES[resolvedAppTheme(displaySettings)].label.en : APP_THEMES[resolvedAppTheme(displaySettings)].label.ja}</Text></View><MaterialIcons name="link" size={20} color={palette.muted} /></View><View style={[styles.widgetEditor, { borderTopColor: palette.border }]}><WidgetMiniPreview english={english} palette={palette} backgroundOpacity={previewBg} cardOpacity={previewCard} backgroundStyle={previewBgStyle} /><View style={appearanceStyles.bgStyleSection}><Text style={[styles.settingTitle, { color: palette.text, marginBottom: 8 }]}>{t("背景スタイル", "Background style")}</Text><BackgroundStyleSelector english={english} selected={previewBgStyle} isPlus={Boolean(isPlus)} onSelect={(key) => { setLocalBgStyle(key); onBackgroundStyle(key); }} onOpenPlus={onOpenPlus} /></View><OpacitySlider english={english} label={t("背景とタイトル", "Background & title")} detail={t("Widget全体とタイトル行に同じ透過率を使います", "Used by the whole widget and its title row")} value={bgValue} onChange={(val) => { setCommittedBgOpacity(val); onBackgroundOpacity(val); }} onDrag={setDragBgOpacity} isPlus={Boolean(isPlus)} onOpenPlus={onOpenPlus} /><OpacitySlider english={english} label={t("項目の背景", "Item row background")} detail={t("Todo・習慣の行だけを独立して調整します", "Adjusts Todo and Habit rows separately")} value={cardValue} onChange={(val) => { setCommittedCardOpacity(val); onCardOpacity(val); }} onDrag={setDragCardOpacity} isPlus={Boolean(isPlus)} onOpenPlus={onOpenPlus} /></View></View>
  </>;
}


function PlusPanel({ english, isIOS, isPlus, price, status, reason, themeSetName, themeSets, onNameChange, onSaveSet, onApplySet, onRemoveSet, onPurchase, onRestore, onRefresh, onManage, onOpenPaywall }: { english: boolean; isIOS: boolean; isPlus: boolean; price?: string; status: string; reason?: string; themeSetName: string; themeSets: SavedThemeSet[]; onNameChange: (value: string) => void; onSaveSet: () => void; onApplySet: (set: SavedThemeSet) => void; onRemoveSet: (id: string) => void; onPurchase: () => void; onRestore: () => void; onRefresh: () => void; onManage: () => void; onOpenPaywall?: () => void }) {
  const t = (ja: string, en: string) => english ? en : ja;
  const storeName = isIOS ? "App Store" : "Google Play";
  const palette = useFocusPalette();
  return <><InfoCard icon="workspace-premium" title={isPlus ? t("Plusを利用中です", "Plus is active") : t("必要になったらPlusへ", "Upgrade only when useful")} detail={isPlus ? t("すべての機能が無制限になり、文字・背景のProスタイルやテーマセットを保存できます。", "All item limits are removed and you can use Pro fonts, backgrounds, and saved theme sets.") : t("無料版はTodo・習慣・メモ各2件、制限アプリ5件、標準スタイルが使えます。", "Free includes 2 tasks, habits, and notes each plus 5 limited apps.")} />
    {!isPlus && onOpenPaywall ? (
      <TouchableOpacity onPress={onOpenPaywall} activeOpacity={0.8} style={[styles.primaryButton, { backgroundColor: palette.primary, marginBottom: 12 }]}>
        <Text style={[styles.primaryButtonText, { color: palette.isDark ? palette.background : COLORS.white }]}>
          {t("👑 Proアップグレード詳細を見る", "👑 View Pro Upgrade Details")}
        </Text>
      </TouchableOpacity>
    ) : null}
    <View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.border }]}><View style={styles.planGrid}><PlanColumn title={t("無料", "Free")} palette={palette} features={[t("Todo・習慣・メモ 各2件", "2 tasks, habits & notes"), t("制限アプリ 5件", "5 limited apps"), t("標準フォントのみ", "Standard font only"), t("無地背景のみ", "Solid background only"), t("基本カラーテーマ", "Basic color themes")]} /><PlanColumn title="Plus" highlighted palette={palette} features={[t("すべて無制限", "Unlimited items"), t("制限アプリ 無制限", "Unlimited limited apps"), t("厳選フォント（3種）", "3 premium fonts"), t("カスタム背景（3種）", "3 custom backgrounds"), t("テーマセットの保存", "Saved theme sets")]} /></View><Text style={[styles.earlyNoticeText, { color: palette.muted }]}>{t("※ 早期完了は設定時間前に1回だけ完了する別の商品です。", "※ Early completion is a separate single-use item.")}</Text>{isPlus ? <View style={styles.inlineActions}><TouchableOpacity onPress={onManage} style={[styles.smallButton, { backgroundColor: palette.primary }]}><Text style={[styles.smallButtonText, { color: palette.isDark ? palette.background : COLORS.white }]}>{t("管理", "Manage")}</Text></TouchableOpacity><TouchableOpacity onPress={onRestore} style={styles.textAction}><Text style={[styles.textActionLabel, { color: palette.primary }]}>{t("購入を復元", "Restore purchases")}</Text></TouchableOpacity></View> : status === "eligible" ? <><TouchableOpacity onPress={onPurchase} style={[styles.primaryButton, { backgroundColor: palette.primary }]}><Text style={[styles.primaryButtonText, { color: palette.isDark ? palette.background : COLORS.white }]}>{price ? t(`${price} でPlusを開始`, `Get Plus for ${price}`) : t("Plusを開始", "Get Plus")}</Text></TouchableOpacity><TouchableOpacity onPress={onRestore} style={styles.textButton}><Text style={[styles.textButtonLabel, { color: palette.muted }]}>{t("以前の購入を復元", "Restore previous purchase")}</Text></TouchableOpacity></> : <><Text style={[styles.mutedCopy, { color: palette.muted }]}>{reason === "NATIVE_BUILD_REQUIRED" ? t(`${storeName}からインストールしたFocus Flowで購入できます。`, `Purchases are available in the Focus Flow build installed from ${storeName}.`) : t("ストアの商品情報を確認できません。商品登録後に再確認してください。", "Store product information is unavailable. Check again after product setup.")}</Text><TouchableOpacity onPress={onRefresh} style={[styles.smallButton, { backgroundColor: palette.primary }]}><Text style={[styles.smallButtonText, { color: palette.isDark ? palette.background : COLORS.white }]}>{t("ストア情報を再確認", "Check store status")}</Text></TouchableOpacity></>}</View>
    {isPlus ? <><SectionTitle title={t("テーマセット（👑 Pro）", "Theme sets (👑 Pro)")} detail={t("今の配色・文字・ウィジェット設定を名前付きで保存します。", "Save the current colors, type, and widgets under a name.")} /><View style={[styles.card, { backgroundColor: palette.surface, borderColor: palette.border }]}><View style={styles.composer}><TextInput value={themeSetName} onChangeText={onNameChange} placeholder={t("例：平日の集中", "e.g., Weekday focus")} placeholderTextColor={palette.muted} style={[styles.themeInput, { backgroundColor: palette.elevated, color: palette.text }]} /><TouchableOpacity onPress={onSaveSet} style={[styles.smallButton, { backgroundColor: palette.primary }]}><Text style={[styles.smallButtonText, { color: palette.isDark ? palette.background : COLORS.white }]}>{t("保存", "Save")}</Text></TouchableOpacity></View>{themeSets.map((set) => <View key={set.id} style={[styles.savedSet, { borderTopColor: palette.border }]}><TouchableOpacity onPress={() => onApplySet(set)} style={styles.savedSetCopy}><Text style={[styles.savedSetName, { color: palette.text }]}>{set.name}</Text><Text style={[styles.selectorDetail, { color: palette.muted }]}>{english ? APP_THEMES[set.appTheme].label.en : APP_THEMES[set.appTheme].label.ja}</Text></TouchableOpacity><TouchableOpacity onPress={() => onRemoveSet(set.id)}><MaterialIcons name="close" size={19} color={COLORS.error} /></TouchableOpacity></View>)}</View></> : null}</>;
}
...
function RoutineEditor({ schedule, expanded, apps, loading, english, onToggle, onChange, onRemove }: { schedule: GateSchedule; expanded: boolean; apps: LaunchableApp[]; loading: boolean; english: boolean; onToggle: () => void; onChange: (input: Partial<GateSchedule>) => void; onRemove: () => void }) { const t = (ja: string, en: string) => english ? en : ja; const palette = useFocusPalette(); const [appsOpen, setAppsOpen] = useState(false); const packages = schedule.blockedPackages ?? []; return <View style={[styles.routine, { borderBottomColor: palette.border }]}><TouchableOpacity onPress={onToggle} style={styles.selectorRow}><View style={[styles.selectorIcon, { backgroundColor: palette.primarySoft }]}><MaterialIcons name="schedule" size={20} color={palette.primary} /></View><View style={styles.selectorCopy}><Text style={[styles.selectorTitle, { color: palette.text }]}>{schedule.label}</Text><Text style={[styles.selectorDetail, { color: palette.muted }]}>{formatDays(schedule.days, english)} · {schedule.startTime}–{schedule.endTime}</Text></View><Switch value={schedule.enabled} onValueChange={(enabled) => onChange({ enabled })} trackColor={{ false: "#CCD7D1", true: "#91C3B3" }} thumbColor={schedule.enabled ? COLORS.forest : "#F7F8F5"} /></TouchableOpacity>{expanded ? <View style={[styles.routineEditor, { borderTopColor: palette.border }]}><TextInput value={schedule.label} onChangeText={(label) => onChange({ label })} style={[styles.nameInput, { color: palette.text, backgroundColor: palette.elevated, borderColor: palette.border }]} placeholderTextColor={palette.muted} placeholder={t("日課の名前", "Routine name")} /><View style={styles.dayPicker}>{["日", "月", "火", "水", "木", "金", "土"].map((label, index) => <TouchableOpacity key={`${label}-${index}`} onPress={() => onChange({ days: toggleNumber(schedule.days, index) })} style={[styles.dayChip, { backgroundColor: palette.elevated }, schedule.days.includes(index) && { backgroundColor: palette.primary }]}><Text style={[styles.dayChipText, { color: palette.muted }, schedule.days.includes(index) && { color: palette.isDark ? palette.background : COLORS.white }]}>{english ? ["S", "M", "T", "W", "T", "F", "S"][index] : label}</Text></TouchableOpacity>)}</View><View style={styles.timeRow}><TimeStepper value={schedule.startTime} english={english} label={t("開始", "Start")} onChange={(startTime) => onChange({ startTime })} /><TimeStepper value={schedule.endTime} english={english} label={t("終了", "End")} onChange={(endTime) => onChange({ endTime })} /></View><TouchableOpacity onPress={() => setAppsOpen((value) => !value)} style={styles.textAction}><Text style={[styles.textActionLabel, { color: palette.primary }]}>{appsOpen ? t("この時間帯のアプリ選択を閉じる", "Hide schedule app selection") : t("この時間帯だけに追加するアプリ", "Add apps for this schedule only")}</Text></TouchableOpacity>{appsOpen ? <RoutineAppPicker apps={apps} loading={loading} selected={packages} english={english} onChange={(blockedPackages) => onChange({ blockedPackages })} /> : null}<TouchableOpacity onPress={onRemove} style={styles.deleteAction}><Text style={styles.deleteActionLabel}>{t("この時間帯を削除", "Delete this schedule")}</Text></TouchableOpacity></View> : null}</View>; }
...
function FontChoice({ item, selected, english, palette, isPlus, onPress }: { item: (typeof APP_FONT_OPTIONS)[number]; selected: boolean; english: boolean; palette: AppPalette; isPlus?: boolean; onPress: () => void }) {
  const isPro = item.id !== "system";
  return (
    <TouchableOpacity
      onPress={onPress}
      style={[
        appearanceStyles.fontChoice,
        {
    ...
    >
      <View style={appearanceStyles.fontChoiceCopy}>
        <Text style={[styles.fontSample, { color: palette.text }, getAppFontStyle(item.id)]}>
          {english ? item.sample.en : item.sample.ja}
        </Text>
        <View style={appearanceStyles.fontChoiceLabelRow}>
          <Text style={[styles.choiceLabel, { color: palette.text }]}>
            {english ? item.label.en : item.label.ja}
          </Text>
          {isPro ? (
            <View
              style={[
                appearanceStyles.bgStyleProBadge,
                {
                  backgroundColor: isPlus ? palette.primarySoft : (palette.isDark ? "#3E2A18" : "#FFF4E3"),
                  borderColor: isPlus ? palette.primary : "#E5A93C",
                },
              ]}
            >
              <Text style={[appearanceStyles.bgStyleProBadgeText, { color: isPlus ? palette.primary : "#B87A10" }]}>
...
const appearanceStyles = StyleSheet.create({
  ...
  fontChoiceLabelRow: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 3 },
