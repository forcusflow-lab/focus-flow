...
export default function SettingsScreen() {
  ...
  const addSchedule = () => setGateConfig({ schedules: [...gateConfig.schedules, { id: `schedule-${Date.now()}`, label: `時間帯 ${gateConfig.schedules.length + 1}`, enabled: true, days: [1, 2, 3, 4, 5], startTime: "09:00", endTime: "18:00" }] });
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <SectionLabel title="有効にする時間帯" detail="時間帯を登録しない場合は、集中ルールを常時適用します" />
            <View style={styles.card}>
              ...
              {gateConfig.schedules.map((schedule) => <ScheduleCard key={schedule.id} schedule={schedule} onChange={(input) => updateSchedule(schedule.id, input)} onRemove={() => removeSchedule(schedule.id)} />)}
