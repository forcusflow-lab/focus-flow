...
const AppSelectionRow = memo(function AppSelectionRow({ app, selected, onPress, selectable = true, blockedByLimit = false }: { app: LaunchableApp; selected: boolean; onPress?: () => void; selectable?: boolean; blockedByLimit?: boolean }) {
  ...
  const content = <>
    ...
    <View style={styles.appCopy}><Text style={[styles.appName, { color: palette.text }]} numberOfLines={1}>{app.label}</Text><Text style={[styles.appPackage, { color: palette.muted }]} numberOfLines={1}>{app.packageName}</Text></View>
  ...
  const rowStyle = [appSelectionStyles.row, { backgroundColor: selected ? palette.primarySoft : palette.surface, borderBottomColor: palette.border }];
...
function AppPicker({ apps, loading, selected, onSelect, canSelect, english }: { apps: LaunchableApp[]; loading: boolean; selected: string[]; onSelect: (value: string) => void; canSelect: (value: string) => boolean; english: boolean }) {
  ...
  return <View style={[styles.appList, { borderTopColor: palette.border }]}>
    ...
    {visible.length ? <FlatList data={visible} renderItem={renderItem} keyExtractor={(item) => item.packageName} style={appSelectionStyles.list} contentContainerStyle={appSelectionStyles.listContent} initialNumToRender={12} maxToRenderPerBatch={12} windowSize={7} removeClippedSubviews={Platform.OS === "android"} nestedScrollEnabled keyboardShouldPersistTaps="handled" getItemLayout={(_data, index) => ({ length: 72, offset: 72 * index, index })} /> : <Text style={[styles.mutedCopy, { color: palette.muted }]}>{t("一致するアプリがありません。", "No matching apps.")}</Text>}
  </View>;
}
...
function RoutineAppPicker({ apps, loading, selected, onChange }: { apps: LaunchableApp[]; loading: boolean; selected: string[]; english: boolean; onChange: (value: string[]) => void }) { const palette = useFocusPalette(); if (loading) return <View style={styles.loading}><ActivityIndicator color={palette.primary} /></View>; return <View style={[styles.appList, { borderTopColor: palette.border }]}>{apps.map((app) => { const active = selected.includes(app.packageName); return <AppSelectionRow key={app.packageName} app={app} selected={active} onPress={() => onChange(active ? selected.filter((item) => item !== app.packageName) : [...selected, app.packageName])} />; })}</View>; }
...
const appSelectionStyles = StyleSheet.create({
  row: { height: 72, flexDirection: "row", alignItems: "center", gap: 10, borderBottomWidth: StyleSheet.hairlineWidth, paddingHorizontal: 12 },
  appTile: { width: 36, height: 36, alignItems: "center", justifyContent: "center", borderRadius: 12 },
  appTileLabel: { fontSize: 16, lineHeight: 20, fontWeight: "900" },
  list: { maxHeight: 360 },
  listContent: { paddingBottom: 4 },
  state: { width: 56, alignItems: "center", justifyContent: "center", gap: 1 },
  stateText: { fontSize: 9, lineHeight: 12, fontWeight: "800" },
});
