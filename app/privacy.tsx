...
export default function PrivacyScreen() {
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.card}><View style={styles.icon}><MaterialIcons name="bug-report" size={20} color={COLORS.forest} /></View><View style={styles.copy}><Text style={styles.cardTitle}>{english ? "Share a bug or improvement" : "不具合・改善案の共有"}</Text><Text style={styles.cardText}>{english ? "Use the Google Play testing feedback channel or your invite's beta contact. Include device model, Android version, and steps to reproduce. Report an incorrect restriction or a state you cannot exit as soon as possible." : "Google Playのテスト用フィードバック、または案内されたベータ窓口から、端末名・Androidバージョン・再現手順を添えてお知らせください。解除できない状態や誤った制限は最優先で報告してください。"}</Text></View></View>
